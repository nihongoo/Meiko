...
function ManageTarget() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await fetch(apiURL.target.all);
                if (!response.ok) {
    ...
    const currentItems = target.slice(indexOfFirstItem, indexOfLastItem);

    const totalPages = Math.ceil(target.length / itemsPerPage);
        ...
        <div className='border bg-light rounded-3'>
            <div className='p-3'>
                ...
                <DataTable
                    ...
                    onChangeData={handleChangeData}
                    apiURLDel={apiURL.target.delete}
                    apiURLEdit={apiURL.target.edit}
                />
