...
function ManageTarget() {
  ...
  const currentPage = 1;
  const { data: targets, loading, error, refetch } = useFetchData(apiURL.target.all);
  ...
  const handleCreate = async () => {
    try {
      const res = await fetch(apiURL.target.create, {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (updatedItem) => {
    try {
      const res = await fetch(`${apiURL.target.edit}${updatedItem.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (data) => {
    try {
      const res = await fetch(`${apiURL.target.delete}${data.id}`, { method: 'DELETE' });
  ...
  return (
    <Box sx={{ p: 3, bgcolor: '#fff', borderRadius: 2 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <SearchInput ApiURL={apiURL.target.search} onSearch={handleSearch} />
        <Box>
