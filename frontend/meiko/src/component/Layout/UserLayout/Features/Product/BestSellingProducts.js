    ...
    fetch('https://localhost:7172/api/Product/Get-All')
      ...
      .then((data) => {
        ...
        const sortedProducts = data
          .filter(product => product.Product_detail[0]?.sale_products.length > 0) // Lọc sản phẩm có giảm giá
          .sort((a, b) => {
            const aDiscountedPrice = getDiscountedPrice(a.Product_detail[0]?.sale_products);
            const bDiscountedPrice = getDiscountedPrice(b.Product_detail[0]?.sale_products);
            const aOriginalPrice = a.Product_detail[0]?.price;
            const bOriginalPrice = b.Product_detail[0]?.price;

            const aDiscountPercent = aDiscountedPrice
              ? Math.round(((aOriginalPrice - aDiscountedPrice) / aOriginalPrice) * 100)
              : 0;
            const bDiscountPercent = bDiscountedPrice
              ? Math.round(((bOriginalPrice - bDiscountedPrice) / bOriginalPrice) * 100)
              : 0;

            return bDiscountPercent - aDiscountPercent; // Sắp xếp theo phần trăm giảm giá từ cao đến thấp
          });
  ...
  return (
    <div className={styles.featuredProducts} style={{ marginTop: '70px' }}>
      <div className="container">
        <Title titleText={"Sản phẩm giảm giá hấp dẫn"} />
